...
import { imageSchema, ImageSchema } from "@/schema/image";
...
export const AddUserImage = ({ profileImage, className }: AddUserImageProps) => {
  ...
  const { mutate: uploadProfileImage, isPending } = useMutation({
    mutationFn: async (profileImage: string) => {
      const { data } = await axios.post(`/api/profile/profile-image`, {
        profileImage,
      });
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <div className="flex mt-5 w-full justify-center items-center gap-4">
                ...
                >
                  {isDeleting ? <Loading /> : <Trash size={18} />}
                </Button>
                <Button
                  type="submit"
