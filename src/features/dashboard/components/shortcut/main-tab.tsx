"use client";

import { Workspace } from "@prisma/client";
import { DateRange } from "react-day-picker";
import { EmojiSelector } from "@/components/common/emoji-selector";
import TextareaAutosize from "react-textarea-autosize";
import { CalendarTask } from "./calendar-task";
import { ActiveWorkspaceInfo } from "./active-workspace-info";
import { useTranslations } from "next-intl";
import { Warning } from "@/components/ui/warning";

type MainTabProps = {
  renderedEmoji: string | string[];
  activeWorkspace: Workspace | null;
  date: DateRange | undefined;
  title: string;
  onSelectEmojiAction: (emoji: string) => void;
  onChangeTabAction: (tab: "main" | "workspaces") => void;
  onSelectedDateAction: (date: DateRange | undefined) => void;
  onChangeTitleAction: (title: string) => void;
}

export const MainTab = ({
  renderedEmoji,
  activeWorkspace,
  date,
  title,
  onChangeTabAction,
  onChangeTitleAction,
  onSelectEmojiAction,
  onSelectedDateAction,
}: MainTabProps) => {
  const t = useTranslations("TASK_SHORTCUT.MAIN_TAB");

  if (!activeWorkspace) {
    return (
      <Warning blue className="my-0">
        <p>{t("WARNING")}</p>
      </Warning>
    );
  } else {
    return (
      <>
        <div className="w-full flex flex-col gap-4 bg-background/70 border border-border p-3 rounded-md shadow-sm">
          <div className="flex gap-4 w-full items-center">
            <EmojiSelector
              className="w-fit"
              onSelectedEmojiAction={onSelectEmojiAction}
              align="center"
            >
              <span className="w-16 h-16 rounded-lg bg-secondary flex justify-center items-center text-3xl">
                {renderedEmoji}
              </span>
            </EmojiSelector>
            <TextareaAutosize
              value={title}
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => {
                onChangeTitleAction(e.target.value);
              }}
              placeholder={t("PLACEHOLDER")}
              className="w-full resize-none appearance-none overflow-hidden bg-transparent placeholder:text-muted-foreground text-2xl font-semibold focus:outline-none max-h-28 overflow-y-auto scrollbar-thin scrollbar-thumb-muted-foreground scrollbar-track-secondary"
            />
          </div>
          <CalendarTask
            date={date}
            onSelectedDateAction={onSelectedDateAction}
          />
        </div>
        <div
          onClick={() => {
            if (activeWorkspace) {
              onChangeTabAction("workspaces");
            }
          }}
          className="w-full flex gap-4 items-center justify-between bg-background/70 border border-border p-3 rounded-md shadow-sm cursor-pointer hover:bg-accent transition-colors duration-200"
        >
          <div className="text-muted-foreground">
            <p>{t("WORKSPACE")}</p>
          </div>
          <div>
            {activeWorkspace ? (
              <ActiveWorkspaceInfo workspace={activeWorkspace} />
            ) : (
              ""
            )}
          </div>
        </div>
      </>
    );
  }
};
