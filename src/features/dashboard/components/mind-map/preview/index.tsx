...
import { LinkTag } from "@/features/dashboard/components/task-edit/link-tag";
...
import { ExtendedMindMap } from "@/features/dashboard/types/mind-map";
import { UserPermission } from "@prisma/client";
import { Info, Star } from "lucide-react";
import { useFormatter, useTranslations } from "next-intl";
import React, { useState } from "react";
import { MindMapCardPreviewOptions } from "./options";
    ...
    <Card className="h-full">
            ...
            <div className="w-full flex justify-between items-center">
              ...
              <div className="absolute top-5 right-5 sm:static">
                <MindMapCardPreviewOptions
                  ...
                  onSetIsSavedAction={onSetIsSavedAction}
                  userRole={userRole}
                />
              </div>
            </div>
      ...
      <CardFooter className="w-full flex flex-col sm:flex-row items-center justify-center gap-2 text-xs mt-4 sm:mt-0">
        <div className="flex items-center">
          <p>{t("CREATOR_INFO")}</p>
          <HoverUserInfo user={mindMap.creator} />
        </div>
